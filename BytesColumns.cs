...
namespace Innovoft.IO
{
	public sealed class BytesColumns : SplitColumns<BytesColumn, byte>
	{
		...
		public bool TryToString(Encoding encoding, int offset, out string value)
		{
			if (offset >= columns.Count)
			{
				value = null;
			...
			var column = columns[offset];
			value = column.ToString(encoding);
			return true;
		}

		public string TryToString(Encoding encoding, int offset)
		{
			if (offset >= columns.Count)
			{
				return null;
		...
		public bool TryToString(Decoder decoder, int offset, out string value)
		{
			if (offset >= columns.Count)
			{
				value = null;
			...
			var column = columns[offset];
			value = column.ToString(decoder);
			return true;
		}

		public string TryToString(Decoder decoder, int offset)
		{
			if (offset >= columns.Count)
			{
				return null;
